...
EnnemiesHolder.prototype.rotateEnnemies = function(){
  for (var i=0; i<this.ennemiesInUse.length; i++){
    ...
    var diffPos = airplane.mesh.position.clone().sub(ennemy.mesh.position.clone());
       // document.getElementById('data').getElementsByTagName('p')[1].innerHTML=ennemy.mesh.position.x;

   
    var d = diffPos.length();
    if (d<game.ennemyDistanceTolerance){
      particlesHolder.spawnParticles(ennemy.mesh.position.clone(), 15, this.ennemiesInUse[i+1].mesh.material.color, 3);
...
function loop(){
  ...
  oldTime = newTime;
  document.getElementById('data').getElementsByTagName('p')[0].innerHTML = deltaTime;
